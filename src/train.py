...
def parse_args():
    ...
    ap.add_argument("--preprocess", required=True, choices=["D1", "D2", "D3", "D4"])
...
def main():
    ...
    if not Path(args.split_csv).exists():
        print(f"[split] creating {args.split_csv} (seed={args.seed})")
        make_split(args.data_root, args.split_csv, seed=args.seed)
